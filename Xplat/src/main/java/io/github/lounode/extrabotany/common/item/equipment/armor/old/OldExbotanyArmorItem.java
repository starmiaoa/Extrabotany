...
public class OldExbotanyArmorItem extends StarryIdolArmorItem implements ManaDiscountArmor {
	...
	public enum Variant {
		...
		Variant(String id, Supplier<ArmorMaterial> material, String armorTexture, Supplier<ItemStack[]> armorSet) {
			this.id = id;
	...
	protected UUID modifierUuid(String name) {
		ResourceLocation id = prefix(variant.id + "_" + name + "." + type.getName());
