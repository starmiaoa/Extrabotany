...
public class CosmicCarKeyItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		UfoEntity ufo = new UfoEntity(level, hit.getLocation().x, hit.getLocation().y, hit.getLocation().z);
		ufo.setYRot(player.getYRot());
		if (!level.noCollision(ufo, ufo.getBoundingBox().inflate(-0.1D))) {
