...
public class FlyingBoatModel extends EntityModel<FlyingBoatEntity> {
	...
	public void setupAnim(FlyingBoatEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		animatePaddle(entity, this.leftPaddle, 0, limbSwing);
		animatePaddle(entity, this.rightPaddle, 1, limbSwing);
	}

	private static void animatePaddle(FlyingBoatEntity entity, ModelPart paddle, int side, float partialTick) {
		float rowingTime = entity.getRowingTime(side, partialTick);
		paddle.xRot = Mth.lerp((Mth.sin(-rowingTime) + 1F) / 2F, -1.0471976F, -0.2617994F);
		paddle.yRot = Mth.lerp((Mth.sin(-rowingTime + 1F) + 1F) / 2F, -Mth.HALF_PI / 2F, Mth.HALF_PI / 2F);
		if (side == 1) {
