...
public class FlowerWeaponEntity extends ThrowableItemProjectile {
	...
	public void tick() {
		...
		if (!this.level().isClientSide() && owner instanceof Player player) {
			ItemStack kingGarden = findKingGarden(player);
			boolean newCharging = kingGarden != null && KingGardenItem.isCharging(kingGarden);
		...
		if (charging) {
			...
			if (liveTicks < delay) {
				...
				this.level().playSound(null, this.blockPosition(), SoundEvents.TRIDENT_THROW, SoundSource.PLAYERS, 1.4F, 0.1F + this.random.nextFloat() * 3F);
			}
